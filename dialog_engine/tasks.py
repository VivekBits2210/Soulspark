...
def summarizer(client, components, chat_history_record, usage_record, keep_limit=5):
    ...
    messages, customizations = components.generate_summarization_prompt(
        keep_limit=keep_limit, summary_index=chat_history_record.summary_index
    )
    if len(messages) == 1:
        return
    ...
    messages, customizations = components.consolidate_summarization_prompt(
        chat_history_record.user_summary
    )
    client.customize_model_parameters(customizations)
    ...
    messages, customizations = components.consolidate_summarization_prompt(
        chat_history_record.bot_summary
    )
    client.customize_model_parameters(customizations)
    consol_summary, cost = client.generate_reply(messages)
    summarizer_tokens += cost
    ...
    chat_history_record.summary_index = max(
        -1, len(chat_history_record.history) - keep_limit - 1
    )
