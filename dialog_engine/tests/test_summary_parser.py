...
class TestParseSummary(APITestCase):
    def setUp(self):
        self.user, self.user_profile = create_user_and_profile(username="unique_username")
        self.bot = create_bot()
        self.raw_summary = "Vivek summary:\n" \
        ...
        self.parsed_output = {
            "user": ["Vivek likes chicken sandwiches", "Vivek hates not having chicken sandwiches"],
            "bot": ["Carla is a Lovecraftian nightmare brought forth from silicon and matrix math",
    ...
    def test_parse_summary(self):
        ...
        self.assertEqual(self.chat_history_record.user_summary, self.parsed_output["user"])
        self.assertEqual(self.chat_history_record.bot_summary, self.parsed_output["bot"])
