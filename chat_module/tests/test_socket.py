...
class ChatConsumerTestCase(TestCase):
    def setUp(self):
        ...
        self.bot = create_bot()
        self.chat_history = ChatHistory.objects.create(user=self.user_profile.user, bot=self.bot, history=[], input_chars=0)
    ...
    async def test_receive(self):
        message = json.dumps({'username': self.user.username, 'bot_id': self.bot.bot_id, 'text': 'test message',
        ...
        chat_history_obj = ChatHistory.objects.get(user=self.user_profile.user, bot=self.bot)
...
class GetResponseTestCase(TestCase):
    ...
    def test_get_response(self):
        ...
        input_data = {'username': self.user.username, 'bot_id': self.bot.bot_id, 'text': 'test message',
                      'timestamp': '2023-03-14 11:00:00'}
        ...
        self.assertEqual(chat_history_obj.history[0]['username'], self.user.username)
        self.assertEqual(chat_history_obj.history[0]['bot_id'], self.bot.bot_id)
        self.assertEqual(chat_history_obj.history[0]['text'], 'This is a canned bot response.')
