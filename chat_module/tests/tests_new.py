...
class ChatModuleTests(TestCase):
    def setUp(self):
        ...
        self.user_profile = UserProfile.objects.create(user=self.user, timezone='America/New_York')

        # Create test bot profile
        self.bot = BotProfile.objects.create(name='TestBot', gender='M', age=25, bio='Test bio', profession='Developer',
        ...
        self.chat_history = ChatHistory.objects.create(user=self.user, bot=self.bot,
                                                       history=[{'message': 'Hi there!', 'sender': 'user'},
                                                                {'message': 'Hello!', 'sender': 'bot'}],
                                                       input_chars=17, level=1.0)
    ...
    def test_fetch_chat_history_lines_not_integer(self):
        self.client.login(username='testuser', password='testpassword')
        response = self.client.get(reverse('fetch_chat_history'), {'bot_id': self.bot.bot_id, 'lines': 'testlines'})
        self.assertEqual(response.status_code, 400)

    def test_fetch_chat_history_lines_less_than_zero(self):
        self.client.login(username='testuser', password='testpassword')
        response = self.client.get(reverse('fetch_chat_history'), {'bot_id': self.bot.bot_id, 'lines': -1})
        self.assertEqual(response.status_code, 400)

    def test_fetch_chat_history_lines_zero(self):
        self.client.login(username='testuser', password='testpassword')
        response = self.client.get(reverse('fetch_chat_history'), {'bot_id': self.bot.bot_id, 'lines': 0})
    ...
    def test_fetch_chat_history_no_history(self):
        self.client.login(username='testuser', password='testpassword')
        response = self.client.get(reverse('fetch_chat_history'), {'bot_id': self.bot.bot_id + 1, 'lines': 5})
        self.assertEqual(response.status_code, 200)
        self.assertEqual(len(response.json()['history']), 0)

    def test_fetch_chat_history_create_chat_history(self):
        self.client.login(username='testuser', password='testpassword')
        bot = BotProfile.objects.create(name='TestBot2', gender='M', age=30, bio='Test bio 2', profession='Engineer',
                                        hobbies='{"hobbies": ["gaming", "traveling"]}',
                                        favorites='{"food": "sushi", "color": "green"}',
                                        physical_attributes='{"height": "175cm", "weight": "70kg"}')
        self.client.login(username='testuser', password='testpassword')
        response = self.client.get(reverse('fetch_chat_history'), {'bot_id': bot.bot_id, 'lines': 5})
        self.assertEqual(response.status_code, 200)
        self.assertEqual(len(response.json()['history']), 0)
    ...
    def test_unmatch_move_chat_history(self):
        ...
        self.assertEqual(response.status_code, 200)
        self.assertTrue(DeletedChatHistory.objects.filter(user=self.user, bot=self.bot).exists())
        self.assertFalse(ChatHistory.objects.filter(user=self.user, bot=self.bot).exists())

    def test_unmatch_multiple_chat_histories(self):
        # Create additional ChatHistory for testing
        chat_history2 = ChatHistory.objects.create(user=self.user, bot=self.bot,
                                                   history=[{'message': 'How are you?', 'sender': 'user'},
                                                            {'message': 'I am good!', 'sender': 'bot'}],
                                                   input_chars=26, level=1.0)

        self.client.login(username='testuser', password='testpassword')
        response = self.client.post(reverse('unmatch'), {'bot_id': self.bot.bot_id})
        self.assertEqual(response.status_code, 200)
        self.assertEqual(DeletedChatHistory.objects.filter(user=self.user, bot=self.bot).count(), 2)
        self.assertFalse(ChatHistory.objects.filter(user=self.user, bot=self.bot).exists())
    ...
    def test_unmatch_deleted_history_intact(self):
        ...
        deleted_chat_history = DeletedChatHistory.objects.filter(user=self.user, bot=self.bot).first()
    ...
    def test_unmatch_no_history(self):
        ...
        self.assertEqual(response.status_code, 200)
        self.assertFalse(ChatHistory.objects.filter(user=self.user, bot=self.bot).exists())
        self.assertFalse(DeletedChatHistory.objects.filter(user=self.user, bot=self.bot).exists())

    def test_fetch_chat_history_no_user_authenticated(self):
        response = self.client.get(reverse('fetch_chat_history'), {'bot_id': self.bot.bot_id, 'lines': 5})
        self.assertEqual(response.status_code, 302)  # Redirects to the login page as user is not authenticated
    ...
    def test_fetch_chat_history_non_integer_lines(self):
        self.client.login(username='testuser', password='testpassword')
        response = self.client.get(reverse('fetch_chat_history'), {'bot_id': self.bot.bot_id, 'lines': 'test'})
        self.assertEqual(response.status_code, 400)
    ...
    def test_fetch_chat_history_wrong_bot_id_type(self):
        self.client.login(username='testuser', password='testpassword')
        response = self.client.get(reverse('fetch_chat_history'), {'bot_id': 'wrong_type', 'lines': 5})
        self.assertEqual(response.status_code, 400)
    ...
    def test_fetch_chat_history_multiple_histories(self):
        ...
        self.assertTrue(ChatHistory.objects.filter(user=self.user, bot=self.bot).exists())
    ...
    def test_unmatch_bot_not_in_history(self):
        ...
        new_bot = BotProfile.objects.create(
            name='NewBot', gender='M', age=30, bio='I am a new bot.',
            profession='Bot Developer', hobbies={}, favorites={},
            physical_attributes={}, profile_image='images/newbot.jpg'
        )

        self.client.login(username='testuser', password='testpassword')
        response = self.client.post(reverse('unmatch'), {'bot_id': new_bot.bot_id})
        self.assertEqual(response.status_code, 200)
        self.assertFalse(DeletedChatHistory.objects.filter(user=self.user, bot=new_bot).exists())
    ...
    def test_fetch_chat_history_multiple_users(self):
        ...
        chat_history2 = ChatHistory.objects.create(
            user=user2, bot=self.bot, history=[{'message': 'How are you?', 'sender': 'user'},
                                               {'message': 'I am good!', 'sender': 'bot'}],
            input_chars=26, level=1.0
        )
    ...
    def test_unmatch_multiple_users(self):
        ...
        self.assertFalse(DeletedChatHistory.objects.filter(user=user2, bot=self.bot).exists())
        ...
        self.assertTrue(DeletedChatHistory.objects.filter(user=user2, bot=self.bot).exists())
    ...
    def test_fetch_chat_history_invalid_lines(self):
        ...
        response = self.client.get(reverse('fetch_chat_history'), {'bot_id': self.bot.bot_id, 'lines': 'invalid'})
        self.assertEqual(response.status_code, 400)
