...
class BotProfileCreateProfileAdminTest(APITestCase):
    ...
    def test_create_profile_admin_view(self):
        ...
        response = self.client.post(url, self.valid_bot_info, content_type='application/json')
        self.assertEqual(response.status_code, status.HTTP_201_CREATED)
        self.assertEqual(BotProfile.objects.count(), 1)
        self.assertTrue(BotProfile.objects.filter(name=self.valid_bot_info["name"]).exists())
        ...
        self.assertEqual(bot.physical_attributes, self.valid_bot_info['physical_attributes'])
